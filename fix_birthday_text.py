...
vzor = r'\{isBirthday\(day\) && \(\s*<div className="absolute inset-0[^"]*">([\s\S]*?)</div>\s*\)\}'

novy = '''{isBirthday(day) && (
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center gap-1 -translate-y-[10%]">
...
if n:
    with open(fp, 'w', encoding='utf-8') as f:
        f.write(novy_kod)
    print("OK - emoji pryc pred jmenem, text vetsi, posun nahoru")
else:
